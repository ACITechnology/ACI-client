  ...
  useEffect(() => {
    ...
    const loadTicket = async () => {
      try {
        ...
        console.error("Erreur loadTicket:", err);
        setLoading(false);
      }
    };
          ...
          <div className="flex flex-col gap-3 md:h-full h-auto overflow-hidden">
            <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-xl p-6 shadow-xl flex-shrink-0">
              ...
              <div className="space-y-1.5 text-xs text-gray-400">
                <div className="flex justify-between">
                  <span>Créé le</span>
                  <span className="text-white">{new Date(ticket.createDate).toLocaleString("fr-FR", { hour12: false })}</span>
                </div>
                <div className="flex justify-between">
                  <span>Modifié le</span>
                  <span className="text-white">{ticket.lastActivityDate ? new Date(ticket.lastActivityDate).toLocaleString("fr-FR", { hour12: false }) : "-"}</span>
                </div>
                <div className="flex justify-between">
                  <span>Statut</span>
                  <span className="text-green-400 font-medium">{ticket.status === 5 ? "Résolu" : "En cours"}</span>
                </div>
              </div>
            ...
            <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-xl p-6 shadow-xl flex flex-col md:flex-grow min-h-[200px] md:min-h-0">
              ...
              <div className="overflow-y-auto pr-2 custom-scrollbar flex-grow">
                <p className="text-gray-400 text-xs leading-relaxed whitespace-pre-wrap break-words">{ticket.description}</p>
              </div>
            </div>
          </div>

          {/* MESSAGERIE (ANCIENNE FORME RÉINTÉGRÉE) */}
            ...
            <div className="relative flex-grow min-h-0 overflow-hidden">
              ...
              >
                {messages.map((msg: any, index: number) => {
                  ...
                  const uniqueKey = msg.id ? `${msg.sourceType}-${msg.id}` : `temp-${index}`;
                  
                  return (
                    <div key={uniqueKey} className="w-full flex justify-center flex-shrink-0">
                      <div className={`w-[98%] p-6 rounded-2xl relative bg-white/5 backdrop-blur-sm ${!isUser ? "border border-white/10" : ""}`}>
                        {/* Bordure dégradée pour l'utilisateur */}
                        {isUser && (
                          <div 
                        ...
                        <div className="flex items-center gap-4 mb-4">
                          <div className={`w-10 h-10 rounded-full flex items-center justify-center text-xs font-bold ${isUser ? "bg-pink-600" : "bg-cyan-600"}`}>
                            {msg.authorName?.charAt(0)}
                          </div>
                          <div className="flex flex-col text-xs text-gray-400">
                            <span className="font-bold text-white">{isUser ? "Moi" : msg.authorName}</span>
                            <span>{new Date(msg.createdAt).toLocaleString('fr-FR', { hour12: false })}</span>
                          </div>
                        </div>
            ...
            <div className="mt-4 flex-shrink-0 pb-2">
              <div className="bg-white/5 border border-white/10 rounded-2xl overflow-hidden focus-within:border-pink-500/50 transition-all shadow-2xl backdrop-blur-md">
                <textarea
                  ...
                  className="w-full bg-black/20 text-gray-300 text-sm p-5 h-[120px] md:h-[160px] focus:outline-none resize-none placeholder:text-gray-600"
                />
